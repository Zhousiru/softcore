...
public class SoftcoreCommand {
        ...
        dispatcher.register(Commands.literal("softcore")
            ...
            .then(Commands.literal("clear")
                .then(Commands.literal("all")
                    .executes(SoftcoreCommand::clearAllCommand))
    ...
    private static int clearCommand(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        for (NameAndId profile : profiles) {
            ...
            BanManager.clearPlayer(uuid);
            ctx.getSource().sendSuccess(() -> Component.literal("已清除 " + name + " 的数据并解封"), true);
        }
        return Command.SINGLE_SUCCESS;
    }

    private static int clearAllCommand(CommandContext<CommandSourceStack> ctx) {
        int count = BanManager.clearAllPlayers();
        ctx.getSource().sendSuccess(() -> Component.literal("已清除 " + count + " 名玩家的数据"), true);
        return Command.SINGLE_SUCCESS;
    }
